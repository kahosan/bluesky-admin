...
export const EzvizPage = () => {
  ...
  const [nvrProps, setNVRProps] = useState<{ deviceSerial: string; status: string; addTime: string }[]>([]);
  const isCreateChildTable = useRef<Record<string, boolean>>({});
  const tdRef = useRef<Record<string, HTMLDivElement>>({});

  // 为了给嵌套的表格加动画
  const channelView = useCallback((deviceSerial: string, status: string, addTime: string) => {
